...
class Bot:
    ...
    def fight(self, current_game_state, player):
        ...
        if self.remaining_code:
            ...
            columns = [
                "health", "x", "y", "is_jumping", "is_crouching", "move_id",
                "enemy_health", "enemy_x", "enemy_y"
            ]
            ...
            if commands:
                ...
                print(f"[ WARNING] Unknown action '{pred_label}'. Performing fallback.")
                self.remaining_code = [">", "!>"]
                self.execute_step(p)
    ...
    def execute_step(self, player):
        ...
        # Mapping input codes to button actions
